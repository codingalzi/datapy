...
# 구글 코랩을 사용하다 보면 특정 라이브러리를 추가 설치해야하는 경우가 
# 발생할 수 있는데 그럴 때는 `pip` 파이썬 라이브러리 관리자를 
# 이용하여 설치하면 된다.
# 예를 들어 웹 크롤링에 유용한 `beautifulsoup4` 라이브러리를 설치하려면 아래 명령문을 주피터 노트북 코드셀에서 실행한다.
# 
# ```bash
...
# 파이썬 코드 작성과 실행에 필요한 모든 라이브러리를 설치, 관리하는 게 권장된다.
...
# 강의노트에 포함된 모든 코드를 실행할 수 있는 
# `pydata-book` 콘다 환경을 설정한다.
# 
# **주의:** 달러 기호($)는 터미널에서 명령문을 입력하는 
# 프롬프트<font size='2'>prompt</font>를 가리킨다.
# 따라서 달러 기호 이후의 명령문만 입력해야 한다.
