...
# 
# __참고:__ 넘파이 어레이와 관련된 많은 기능이 뷰 기능을 이용한다. 이전에 소개한 전치 어레이를 
# 구하는 과정도 뷰를 이용한다.
...
# **참고:** 진정한 흑백사진으로 변경하려면 RGB 정보를 전부 이용하여 하나의 값을 계산해야 한다.
...
# __참고:__ 부울 인덱싱에 사용되는 부울 어레이를 __마스크__(mask)라고 한다.
...
# 이제 `mask`를 이용하여 모든 음수 항목을 0으로 변경할 수 있다.
...
# **팬시 인덱싱**<font size='2'>fancy indexing</font>은 정수를 이용한다. 
...
# **참고:** 팬시 인덱싱은 객체를 항상 새로 생성한다. 즉, 뷰 기능을 이용하지 않는다. 
